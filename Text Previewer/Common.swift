...
final class Common: NSObject {
    ...
    // FROM 1.0.5
    var minimumTumbnailSize: CGFloat    = BUFFOON_CONSTANTS.MIN_THUMB_SIZE
    ...
    func setProperties(_ usePrefs: Bool = true, _ fontName: String = BUFFOON_CONSTANTS.BODY_FONT_NAME) {
        ...
        if usePrefs {
            if let prefs = UserDefaults(suiteName: self.appSuiteName) {
                ...
                self.minimumTumbnailSize = CGFloat(prefs.float(forKey: BUFFOON_CONSTANTS.PREFS_IDS.THUMB_MIN_SIZE))
            }
